...
Last updated: after Phase 6 gameplay-logic implementation. Rendering/UI wiring is a separate, still-open question — see Phase 6 below.

## Current Phase

Phase 6 — Color Expansion MVP. Gameplay logic (Grid, Players, Movement, Territory capture, Elimination, Win condition) is implemented and verified in-session (see below), but not yet reviewed or approved by the project owner, and rendering/UI wiring has not been started (see Phase 6's "Not implemented this session" note). Per Roadmap.md's Development Rules, Phase 7 should not begin until both are resolved.
...
### Phase 6 — Color Expansion MVP (gameplay logic — rendering/UI wiring not included, see below)

Implemented, matching ColorExpansion.md's Grid / Players / Movement / Territory capture / Elimination / Win condition sections, with Character Skills explicitly out of scope per the project owner's instruction (`Skills.ts` remains an empty placeholder):

- `src/simulations/ColorExpansion/Grid.ts` — the grid data structure (`GridState`: a `size` and a mutable `cells[y][x]` array of `string | null` owner ids — `null` means neutral) and every operation on it: `createGrid`, `isInsideGrid`, `getCellOwner`, `claimCell`, `hasAnyNeutralCell`, and `findNextStepTowardNearestNeutralCell` — a breadth-first search returning the first step of the shortest path to the nearest neutral cell. The player's own territory is passable; enemy territory is an impassable wall (see ColorExpansion.md, Movement); a neutral cell is always a goal, never a passable intermediate step, since entering one immediately claims it (see Territory). Returns `null` if no neutral cell is reachable at all — the elimination trigger.
- `src/simulations/ColorExpansion/Config.ts` — `COLOR_EXPANSION_CONFIG`, a `Config<ColorExpansionConfigShape>` (see shared/Config.ts) holding two **temporary placeholder** balance values: `gridSize: 20` and `movementSpeedCellsPerSecond: 4`. Both are explicitly flagged as unplaytested in the file's own doc comment and in Todo.md — per the project owner's instruction not to spend time perfectly balancing a simulation that doesn't exist yet. Every gameplay file reads these two values through this one Config instance, never as inline numbers, so retuning either later never touches Grid.ts or ColorExpansion.ts.
- `src/simulations/ColorExpansion/ColorExpansion.ts` — `createColorExpansionSimulation(players: Player[])`, a factory returning a `Simulation<ColorExpansionState>` (see types/Simulation.ts), ready to be `load()`-ed into a `SimulationEngine` once a real roster of 2–4 `Player`s exists (see "Not implemented this session" below). Also exports `computeColorExpansionStats(state)`, a pure function producing each player's live territory count/percentage for future use by `engine/statistics/StatisticsStore` and the Phase 5 UI components — "Rank" itself is deliberately not computed here, since that's exactly what `engine/statistics/Ranking.ts`'s `descendingBy()` is for.

  `ColorExpansionPlayerState` extends the shared `Player` type (`id`, `slot`, `character`) with this simulation's own movement/territory fields (`x`, `y`, `targetX`/`targetY`, `moveProgress`, `eliminated`) — per Architecture.md's one-canonical-location rule, this lives in ColorExpansion's own folder rather than `/src/types`, since only this simulation uses it.

  `seed` is accepted by `createInitialState(seed)` (required by the `Simulation<TState>` interface) but not yet consumed anywhere: every rule implemented this phase — spawn corners, BFS pathfinding — is fully deterministic without randomness. Verified directly (see below): the same player roster produces identical results regardless of which seed is passed in. This is expected to change once Trickster's random skill (Phase 7) becomes the first actual consumer of a seeded RNG here.

**Verification performed this session** (this environment does have sandboxed network/bash access, unlike the general case Roadmap.md describes for the project owner's own machine):

- `tsc` against a copy of the project's real compiler options (strict, verbatimModuleSyntax, noUnusedLocals, noUnusedParameters, moduleResolution: bundler — copied from tsconfig.app.json) — clean, zero errors.
- `oxlint` with the project's real `.oxlintrc.json` — zero warnings, zero errors, on the new files.
- A compiled runtime check: ran 2-, 3-, and 4-player games to completion at the fixed 1000/60ms timestep and confirmed (a) the same seed always produces identical final per-player territory counts and elimination flags, (b) a *different* seed also produces identical results (expected, since no randomness is consumed yet), (c) every run terminates with no infinite loop — up to roughly 3,000 ticks for a 20×20 grid at the placeholder speed, and (d) the grid always ends either fully claimed or with every player eliminated, matching `isComplete()`'s contract.
- A stress test forcing multiple cell-crossings within a single `update()` call (very large `deltaTimeMs`, to stand in for a much higher future movement speed) and irregular, non-fixed timesteps — both completed correctly with no infinite loop, confirming the movement loop's `while` (rather than a single-step check) holds up regardless of how `movementSpeedCellsPerSecond` eventually gets tuned.

None of this verification code is part of the deliverable — it lived in a throwaway sandbox and is not included in the handed-off files.

**Not implemented this session, and why:**

- **Rendering and App/Menu wiring** — drawing the grid and square players, loading `createColorExpansionSimulation` into a `SimulationEngine` from `App.tsx`, feeding live stats into `StatsPanel`/`WinnerScreen`, and calling `UIManager.showWinner()` on completion. Roadmap.md's own Phase 6 "Implement:" list names only Grid, Players, Movement, Territory capture, Elimination, and Win condition — unlike Phase 2, which explicitly listed renderer work — and the Development Rules say "Implement only the current milestone" and "Do not refactor unrelated systems." Touching `Renderer.ts`, `CharacterRenderer.ts`, `Arena.tsx`, and `App.tsx` to make this visible on screen felt like exactly that kind of unrelated-system change rather than part of the six listed items. Flagging this explicitly, mirroring how Phase 3 flagged `isRunning()` and Phase 5 flagged the still-uncalled `showWinner()`: the project owner should decide whether UI wiring is folded into Phase 6 or done as a short follow-up before Phase 7.
- **Character Skills** — explicitly out of scope per the project owner's instruction. `Skills.ts` remains an empty placeholder for Phase 7.

**Decisions made along the way (flagged for review, same as prior phases' judgment calls):**

- **Spawn corner assignment.** ColorExpansion.md specifies "opposite corners" for 2 players and "three corners" for 3, but never says which three (of four) corners, or which two count as "opposite." Implemented as one fixed winding order — top-left, top-right, bottom-right, bottom-left — with an N-player game using the first N. This single rule satisfies all three documented cases at once: slots 0/1 land on top-left/bottom-right (diagonally opposite, for 2 players), slots 0–2 land on three distinct corners (for 3 players, omitting bottom-left), and slots 0–3 use all four (for 4 players). Which specific corner is omitted for 3 players is the actual judgment call here.
- **Deterministic pathfinding tie-break.** ColorExpansion.md's Movement section says "If multiple paths exist, Character Skills may influence path selection," implying a well-defined default exists before any skill touches it — but doesn't say what that default is. Implemented as a fixed neighbor search order (up, right, down, left) inside the BFS in `findNextStepTowardNearestNeutralCell`, so a given grid state always yields the same chosen step. Skills.ts (Phase 7) is expected to bias or override this, not replace the underlying search.
- **Same-cell race resolution.** Nothing in ColorExpansion.md addresses two players' independently-computed nearest-neutral-cell targets turning out to be the same cell. Implemented as: players are processed in a fixed order every tick (array/slot order), so whichever player is processed first in a given tick claims a contested cell first; the other player's target-validity check (`hasValidTarget`) then detects the now-enemy-owned cell on its own next iteration and recomputes a fresh path. Deterministic for a given roster, but not something the docs specify.
- **State mutation.** `ColorExpansionState` (including the grid's `cells` array) is mutated in place inside `update()` rather than deep-cloned every tick, and the same object reference is returned. `Simulation<TState>.update()`'s contract only requires it to *return* a `TState` — it doesn't mandate immutability — and Skills.md's "never mutates state directly" rule is scoped to individual Skill functions reading shared state, not to a simulation's own `update()` managing its own state. Cloning a few-hundred-cell grid 60 times a second with no consumer that needs a "previous" snapshot would be pure overhead.
- **`Player` type reuse.** `ColorExpansionPlayerState extends Player` (from `types/Player.ts`) rather than redeclaring `id`/`slot`/`character` locally, since that type exists specifically to be shared across simulations' own player-state shapes.

Verified in this session (see "Verification performed" above) but **not yet reviewed or approved by the project owner** — per Roadmap.md's Development Rules, Phase 7 (Color Expansion Skills) should not begin until that review happens, and the rendering/UI wiring question above should be resolved first too.
...
- **(Phase 6)** Grid, spawn, and pathfinding needed a concrete spawn-corner rule and a concrete pathfinding tie-break rule, neither of which ColorExpansion.md fully specifies. Both are documented in the Phase 6 write-up above, along with same-cell race resolution and the choice to mutate state in place rather than clone it every tick.
- **(Phase 6)** Rendering/App/Menu wiring was deliberately left out of this session's scope, even though Roadmap.md's Phase 6 deliverable says "Playable Color Expansion." Roadmap's own Phase 6 "Implement:" list only names the six gameplay-logic items (unlike Phase 2, which explicitly listed renderer work), and the Development Rules say not to refactor unrelated systems or implement ahead of the current milestone. Flagging this as an open question for the project owner rather than deciding it unilaterally either way.
- **(Phase 6)** `seed` is accepted by `createColorExpansionSimulation(...).createInitialState(seed)` per the `Simulation<TState>` interface but is not consumed by anything yet — confirmed directly by running the same roster with two different seeds and getting identical results. This will change the moment Skills.ts (Phase 7) introduces Trickster's random movement bonuses.

## For a New Chat

Read Blueprint.md first, then this file, before anything else.

Phase 1 through Phase 5 files (listed above under Completed Phases) are implemented and were approved by the project owner. Phase 6's gameplay logic (Grid, Players, Movement, Territory capture, Elimination, Win condition — see Phase 6 above) is implemented and was verified in-session (`tsc` with the project's real compiler options, `oxlint` with the project's real config, and a compiled runtime smoke test all passed — see Phase 6 notes) but has **not** yet been reviewed or approved by the project owner, and its rendering/UI wiring is a still-open question (see Phase 6's "Not implemented this session" note). Per Roadmap.md's Development Rules, Phase 7 should not begin until both are resolved.

`src/simulations/ColorExpansion/Grid.ts`, `Config.ts`, and `ColorExpansion.ts` are implemented (see Phase 6). `src/simulations/ColorExpansion/Skills.ts` remains an empty placeholder — Character Skills are Phase 7. Everything else under `src/engine/audio`, `src/engine/recording`, `src/simulations/WeaponClash`, and `src/engine/core/Physics.ts` is still an empty placeholder. A file existing does not mean it is implemented; check actual file contents, not just the file tree, before assuming any phase is complete.

The demo arena/characters wired into `src/components/Arena/Arena.tsx` and used by `src/App.tsx`'s "running" view are still Phase 2 scaffolding, now aspect-ratio aware (Phase 5) but otherwise unchanged. Color Expansion's real `Simulation<ColorExpansionState>` now exists (`createColorExpansionSimulation` in `ColorExpansion.ts`) and is ready to be `load()`-ed into a `SimulationEngine`, but nothing in `App.tsx`/`Arena.tsx` does so yet — the demo should be replaced rather than extended once that wiring is undertaken (Weapon Clash will need the same treatment in Phase 8).

The Menu (`src/menu`), the shared UI screens (`src/components/UI`), the shared presentational primitives (`src/components/Shared`), and the UI phase manager (`src/engine/ui/UIManager.ts`) are all implemented and wired together in `src/App.tsx` — but the "running" phase they lead into still has no real simulation behind it, and there is no automatic path to the "winner" phase (`UIManager.showWinner()` still has no caller). Both become real once the rendering/UI wiring flagged in Phase 6 happens.

`SimulationEngine` still has a full run lifecycle (`start`/`stop`/`restart`/`reset`) and a fixed-timestep simulation update hooked into the tick loop, and now has a real `Simulation<TState>` implementation available to load (`createColorExpansionSimulation`) — but nothing yet calls `load()` with it.

`StatisticsStore<TStats>` (engine/statistics), `Config<T>` (shared/Config.ts), and `AspectRatio.ts` (shared/AspectRatio.ts) are available as reusable infrastructure. Color Expansion now has its own `Config<ColorExpansionConfigShape>` instance (`COLOR_EXPANSION_CONFIG` in `src/simulations/ColorExpansion/Config.ts`, with temporary placeholder values — see Phase 6) and a pure `computeColorExpansionStats(state)` function ready to feed a `StatisticsStore`, but nothing constructs or populates one yet. Weapon Clash's own `Config.ts` (still an empty placeholder) is expected to build a `Config<TheirOwnShape>` once that simulation's balance values are decided (Todo.md, Balance) — likely with the same "temporary placeholder, flagged in Todo.md" treatment Color Expansion just got, rather than waiting for final numbers.
